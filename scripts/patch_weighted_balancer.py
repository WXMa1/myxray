#!/usr/bin/env python3
"""Patch XTLS/Xray-core to add a weightedRoundRobin routing balancer.

Design goals:
- no line-number based patching;
- idempotent (safe to run repeatedly);
- anchors on stable identifiers / switch cases rather than exact line positions;
- avoids touching config.proto/config.pb.go by reusing StrategyLeastLoadConfig.Costs
  as the typed-message carrier for StrategyWeight entries.
"""
...
def patch_router_strategy() -> None:
    ...
    if not re.search(r'(?m)^\s*strategyWeightedRoundRobin\s+(?:string\s*)?=\s*"weightedroundrobin"\s*$', text):
        # Add a new strategy identifier immediately after the existing strategy
        # constants block member. This is identifier-based, not line-based.
        text = sub_once(
            text,
    ...
    if not re.search(r'(?m)^\s*strategyWeightedRoundRobin\s*:\s*func\(\)', text):
        # Register settings parser next to the existing leastLoad registration.
        text = sub_once(
            text,
    ...
    if "type strategyWeightedRoundRobinConfig struct" not in text:
        ...
        ''')
        # Prefer the nearby HealthCheckSettings declaration as the insertion
        # anchor. If upstream moves/removes that comment, fall back to the
        # strategyLeastLoadConfig.Build method. Neither path depends on line numbers.
        health_anchor = re.search(
            r'(?m)^//\s*HealthCheckSettings\b[^\n]*$',
            text,
        )
        if health_anchor:
            ...
            build_anchor = re.search(
                r'(?m)^func\s+\(v\s+\*strategyLeastLoadConfig\)\s+Build\s*\(',
                text,
            )
            if not build_anchor:
...
def patch_router_conf() -> None:
    ...
    # Locate the accepted-strategy case containing the known strategy identifiers,
    # then append weightedRoundRobin. Whitespace/order may vary upstream.
    pattern = (
...
def patch_runtime_builder() -> None:
    ...
    # Insert immediately before the existing random branch. Function name and
    # case label are semantic anchors, so unrelated upstream line movement is fine.
...
def main() -> None:
    # Basic repository identity checks; no dependency on a specific commit.
    go_mod = read("go.mod")
    ...
    print("[patch] weightedRoundRobin support applied successfully")
