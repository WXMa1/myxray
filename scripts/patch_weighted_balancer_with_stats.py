#!/usr/bin/env python3
"""Patch XTLS/Xray-core to add weightedRoundRobin + native selection counters.

Compatibility goals:
- preserve the existing WXMa1/myxray weightedRoundRobin behavior exactly;
- no line-number based patching;
- idempotent (safe to run repeatedly);
- anchor on semantic identifiers / switch cases;
- keep the existing StrategyLeastLoadConfig.Costs protobuf carrier;
- add only a StatsService-visible counter after a WRR outbound is selected.

New counter name:
    outbound>>><outbound-tag>>>>wrr>>>selections

The counter is available only when Xray has a real stats.Manager. If Xray runs
without a stats app, weightedRoundRobin continues to work exactly as before and
selection counting becomes a no-op.
"""
...
STRATEGY_GO = dedent(r'''
    ...
    //
    // This strategy also exposes a cumulative per-outbound selection counter via
    // Xray's existing stats.Manager. Counter collection is observational only:
    // failure/unavailability of stats must never change the scheduling result.
    type WeightedRoundRobinStrategy struct {
        ...
        // mu protects only the scheduling state. Keep the scheduling critical
        // section independent from stats registration / counter bookkeeping.
        mu      sync.Mutex
        ...
        selectionCounters       map[string]featurestats.Counter
        selectionStatsAvailable bool
    }
    ...
    func NewWeightedRoundRobinStrategy(settings *StrategyLeastLoadConfig, fallbackTag string) (*WeightedRoundRobinStrategy, error) {
        s := &WeightedRoundRobinStrategy{
            ...
            selectionCounters: make(map[string]featurestats.Counter),
        }
        if settings == nil {
            return s, nil
        }
    ...
    func (s *WeightedRoundRobinStrategy) InjectContext(ctx context.Context) {
        ...
        // Stats are optional for WRR behavior. Xray always provides a stats.Manager
        // feature (a NoopManager if no real stats app is configured), so acquiring
        // it here does not make weightedRoundRobin depend on StatsService.
        common.Must(core.RequireFeatures(s.ctx, func(statsManager featurestats.Manager) error {
            s.statsMu.Lock()
            defer s.statsMu.Unlock()
            s.statsManager = statsManager
            s.selectionCounters = make(map[string]featurestats.Counter)
            _, isNoop := statsManager.(featurestats.NoopManager)
            s.selectionStatsAvailable = !isNoop
            return nil
        }))
    ...
    func (s *WeightedRoundRobinStrategy) PickOutbound(candidates []string) string {
        ...
        // Smooth weighted round robin -- intentionally unchanged from the
        // previously validated WRR patch:
        // current[i] += weight[i]
        // select max(current)
        ...
        // Observational side effect only. Scheduling has already completed, and
        // recordSelection deliberately cannot alter the selected outbound.
        s.recordSelection(selected)
        return selected
    }

    func (s *WeightedRoundRobinStrategy) recordSelection(tag string) {
        if tag == "" {
            return
        }

        s.statsMu.Lock()
        if !s.selectionStatsAvailable || s.statsManager == nil {
            s.statsMu.Unlock()
            return
        }
        if counter, found := s.selectionCounters[tag]; found {
            s.statsMu.Unlock()
            counter.Add(1)
            return
        }
        statsManager := s.statsManager
        s.statsMu.Unlock()

        // Lazy registration keeps the counter set limited to outbounds that were
        // actually selected and avoids touching protobuf/API definitions.
        name := "outbound>>>" + tag + ">>>wrr>>>selections"
        counter, err := statsManager.GetOrRegisterCounter(name)
        if err != nil || counter == nil {
            // A real stats manager should accept this non-empty name. If it does
            // not, fail closed for telemetry only; WRR scheduling remains active.
            s.statsMu.Lock()
            s.selectionStatsAvailable = false
            s.statsMu.Unlock()
            return
        }

        s.statsMu.Lock()
        if existing, found := s.selectionCounters[tag]; found {
            counter = existing
        } else {
            s.selectionCounters[tag] = counter
        }
        s.statsMu.Unlock()
        counter.Add(1)
    }
...
TEST_GO = dedent(r'''
    ...
    func TestWeightedRoundRobinStrategySelectionCounters(t *testing.T) {
        s, err := NewWeightedRoundRobinStrategy(&StrategyLeastLoadConfig{
            Costs: []*StrategyWeight{
                {Match: "node-a", Value: 5},
                {Match: "node-b", Value: 3},
        ...
        s.statsManager = sm
        s.selectionStatsAvailable = true

        candidates := []string{"node-a", "node-b", "node-c"}
        for i := 0; i < 10; i++ {
            if got := s.PickOutbound(candidates); got == "" {
        ...
        for tag, want := range expected {
            counter := sm.GetCounter("outbound>>>" + tag + ">>>wrr>>>selections")
            if counter == nil {
                t.Fatalf("selection counter missing for %s", tag)
            }
            if got := counter.Value(); got != want {
                t.Fatalf("selection counter for %s = %d, want %d", tag, got, want)
            }
        }
    }

    func TestWeightedRoundRobinStrategyStatsFailureDoesNotChangeScheduling(t *testing.T) {
        s, err := NewWeightedRoundRobinStrategy(&StrategyLeastLoadConfig{
            Costs: []*StrategyWeight{
                {Match: "node-a", Value: 2},
                {Match: "node-b", Value: 1},
            },
        }, "")
        if err != nil {
            t.Fatal(err)
        ...
        // No stats manager / counter support: WRR distribution must remain intact.
        s.selectionStatsAvailable = false
        counts := map[string]int{}
        for i := 0; i < 30; i++ {
            counts[s.PickOutbound([]string{"node-a", "node-b"})]++
        }
        if counts["node-a"] != 20 || counts["node-b"] != 10 {
            t.Fatalf("stats-disabled scheduling changed: %#v", counts)
        }
    }
''')
...
def patch_runtime_builder() -> None:
    ...
    block = dedent('''
        case "weightedroundrobin":
            ...
            if !ok {
                return nil, errors.New("not a StrategyLeastLoadConfig for weightedRoundRobin").AtError()
            }
...
def write_new_files() -> None:
    # These are deliberately rewritten to the canonical current version on every
    # run. That upgrades an older WRR-only patch tree to WRR+selection-stats while
    # leaving the three original integration points untouched when already present.
...
def main() -> None:
    ...
    print("[patch] weightedRoundRobin support preserved")
    print("[patch] native WRR selection counters applied")
    print("[patch] counter format: outbound>>><tag>>>>wrr>>>selections")
